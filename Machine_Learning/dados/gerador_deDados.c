...
//Configuracoes
int numero_jogadores=1000;
int numero_deTorneios=1000;
int gerar_dataLogs=0;
//------------//-----------//
...
void shuffle_grupo(jogador *lista)
{
  jogador *temp_1;
  jogador *varredor;
  int n_lista1, n_total, i, j;
  n_total=(numero_populacao(lista)/4);
  for(j=1;j<=n_total;j++)
  {
    n_lista1=(rand()%1000)+1;
    //busca individuo 1
    varredor=lista->anterior;
    for(i=1;i<=n_lista1;i++)
