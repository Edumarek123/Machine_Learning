#ifndef TORNEIORPG_H_INCLUDED
#define TORNEIORPG_H_INCLUDED
...
void analisar_dados(jogador *lista, int nome)
{
    char* classe;
    jogador *temporario;
    FILE *arquivo;
...
void torneios(jogador* populacao)
{
    ...
    for(i=1; i<=numero_deTorneios; i++)
    {
      ...
      if(gerar_dataLogs)
        analisar_dados(vencedores, i);
      if(numero_populacao(populacao)==1)
      {
        imprimi_quantidade_torneios(i);
...
{
  FILE *arquivo;
  arquivo = fopen("vencedor.txt", "a");
  fprintf(arquivo, "Individuo: %d | Classe: %s | Vida: %d | Defesa: %d | Ataque: %d\n", lista->anterior->identificador, lista->anterior->classe, lista->anterior->vida, lista->anterior->defesa, lista->anterior->ataque);
  fclose(arquivo);
}
